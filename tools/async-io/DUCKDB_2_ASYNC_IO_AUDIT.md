...
### First controlled result: 150 ms latency model

Workflow run `35528341919` completed the full five-sample 150 ms A/B before the subsequent zero-latency harness assertion failed. The measured query and settings were identical in both modes: `SUM(id)`, `threads=2`, `async_threads=2`, `read_ahead_depth=4`, external file cache disabled. The sync-XHR baseline had median **6514.095 ms** and p95 **6516.590 ms**; the async-fetch broker had median **2842.640 ms** and p95 **2846.685 ms**. That is a **2.292x median speedup** and **2.289x p95 speedup** under the deterministic 150 ms/request latency model. The stable samples transferred the same 1,647,224 bytes and issued 42 ranged GETs, while measured maximum overlap changed from 1 for the latency-bound sync-XHR baseline to 3 for the broker. Effective transferred-byte throughput on the stable samples was about 2.02 Mbit/s baseline versus 4.63 Mbit/s broker.

The first sample in each mode was shorter and used fewer/coalesced requests (16/22 GETs), so mean latency is not used as the headline metric. Median and p95 are robust to that startup outlier. The zero-delay leg of that run did **not** produce a valid performance result because the harness incorrectly required sync XHR to have `max_overlapping_ranges=1`; with sub-millisecond local responses the server recorded a tiny timing overlap of 2 even though the broker was disabled. Commit `1a502d307d1999bba9b9a3c6d32f5334a2b85584` removes that invalid zero-latency assertion; the query correctness and transport selection remain unchanged.

Follow-up run `35528519665` passed both benchmark legs. At 150 ms, the sync-XHR baseline median was **6520.505 ms** and the broker median **2850.850 ms**, a **2.287x median speedup**; p95 improved from **6523.935 ms** to **2884.710 ms** (**2.262x**). At zero injected latency, raw samples were intentionally not treated as directly comparable because DuckDB coalesced reads differently between trials (14-42 GETs). Restricting comparison to the shared stable request signature of **42 ranged GETs / 1,647,224 bytes**, both transports are effectively neutral at about **480 ms**, i.e. no meaningful broker overhead. The benchmark runner now computes this shared request-signature cohort automatically and uses it for performance gates: latency-bound median speedup must be at least 1.5x, while a zero-latency comparable cohort must not regress by more than 10%.
