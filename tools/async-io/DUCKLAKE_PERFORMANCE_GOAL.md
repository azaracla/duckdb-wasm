...
The immediate next gate is unchanged: one DuckDB SQL query must produce at least two overlapping HTTP 206 Range GETs. Range transport itself is now proven: run `35524900131` issued two correct HTTP 206 ranged GETs, but they were serial (`max_overlapping_ranges=1`) and Chromium reported pthread-pool exhaustion. Because that run used metadata-friendly `COUNT(*)`, the next acceptance query is a real column scan (`SUM(id)` across 20 row groups) with bounded `async_threads=2` and `read_ahead_depth=4`. No DuckLake performance claim is valid before actual overlap is observed.
